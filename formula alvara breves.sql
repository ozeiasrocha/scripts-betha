...
'"                       Pelo presente ALVARÁ, é concedido a(o) Sr.(a) ' + if( isnull(nome),' ',nome) + ' residente à ' + if(isnull(endereco),' ',endereco) +' licença para ' + if(isnull(desc_tipo_obra),' ',desc_tipo_obra) + ', tipo: ' tipo_obra +', com área de:  '+area+' '+ if (isnull(projetos_descricao),' ',projetos_descricao) + ' em terreno com cadastro municipal ' + string(projetos_i_imoveis ) + ' inscrição imobiliária ' + if(isnull(insc),' ',insc) + ' sito à ' + if(isnull(end_obra),' ',end_obra) + ' Bairro ' + if(isnull(bairro),' ',bairro) + ' conforme projeto n.º ' + string( projetos_i_projetos ) + ' aprovado por esta Secretaria em ' + if( not isnull(projetos_data_parecer), string( projetos_data_parecer ), string(projetos_data_entrada) ) + ', de acordo com os Artigos/Leis que dispõe sobre o código de obras deste município.'
