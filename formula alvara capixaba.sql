...
SET area=isnull(dbf_fc_retbce_valor(w_codigo, 699, dbf_ano_base(w_codigo, w_ano, 699, 'opcoes_bce')),0);
IF area = '' THEN
   CALL dbp_raiserror('','INFORME A ÁREA DO ESTABELECIMENTO!');
END IF;  
MESSAGE string ('area ',area) TYPE WARNING TO CLIENT;      

%%Busca opção de categoria
